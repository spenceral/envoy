...
TEST_P(AdminInstanceTest, ConfigDumpFiltersByResource) {
 ...
 "configs": [
  ...
  EXPECT_EQ(Http::Code::OK, getCallback("/config_dump?resource=dynamic_listeners", header_map, response));
  std::string output = response.toString();
  EXPECT_EQ(expected_json, output);
}
...
TEST_P(AdminInstanceTest, ConfigDumpFiltersByMask) {
 ...
 "configs": [
  ...
  EXPECT_EQ(Http::Code::OK, getCallback("/config_dump?mask=dynamic_listeners", header_map, response));
  std::string output = response.toString();
  EXPECT_EQ(expected_json, output);
}
...
TEST_P(AdminInstanceTest, ConfigDumpFiltersByResourceAndMask) {
 ...
 "configs": [
  ...
  EXPECT_EQ(Http::Code::OK, getCallback("/config_dump?resource=static_clusters&mask=cluster.name", header_map, response));
  std::string output = response.toString();
  EXPECT_EQ(expected_json, output);
}
...
TEST_P(AdminInstanceTest, ConfigDumpNonExistentMask) {
 ...
 "configs": [
  ...
  EXPECT_EQ(Http::Code::OK, getCallback("/config_dump?resource=static_clusters&mask=bad", header_map, response));
  std::string output = response.toString();
  EXPECT_EQ(expected_json, output);
}
...
TEST_P(AdminInstanceTest, ConfigDumpResourceNotRepeated) {
  ...
  EXPECT_EQ(Http::Code::BadRequest, getCallback("/config_dump?resource=version_info", header_map, response));
}
